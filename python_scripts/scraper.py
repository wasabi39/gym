...
def find_title(url_number):
    ...
    for title in soup.find_all('title',limit=1):
        title = title.get_text()[17:].replace(r'"','').replace(r"'",'').strip(" ").replace(r",","")
        #print(title)
    return title.lower()
...
def find_jobtype(job_title):
    if "vikar" in job_title:
        return "vikariat"
    if "fast stilling" or "pædagogikum" in job_title:
        ...
        for i in range(63866,66800):
            ...
            if job_title != "undet":
                try:
                    ...
                    subjects = find_subjects(i)

                    jobtype = find_jobtype(job_title)
